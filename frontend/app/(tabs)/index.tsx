...
export default function HomeScreen() {
  ...
  // ✅ CRITICAL: Provide accurate item layout for FlatList performance
  // Each flick is full screen height for snapping/paging
  const getFlickItemLayout = (data: ArrayLike<Flick> | null | undefined, index: number) => ({
    length: screenHeight,
    offset: screenHeight * index,
    index,
  });

  // ✅ CRITICAL: Provide accurate item layout for explore posts
  // Post card in grid estimated height: ~380px (image + header + actions + padding)
  const getPostItemLayout = (data: ArrayLike<Post> | null | undefined, index: number) => ({
    length: 380,
    offset: 380 * Math.floor(index / 2), // 2-column grid
    index,
  });
  ...
  const renderPost = ({ item }: { item: Post }) => (
    <View style={styles.postCardContainer}>
      <View style={[styles.postContainer, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
        <Image
          source={{
            uri: item.secure_url || 'https://via.placeholder.com/400x300',
          }}
  ...
  const renderExploreItem = ({ item }: { item: Post }) => (
    <View style={styles.exploreItemContainer}>
      {/* Username Header - Clickable */}
      <TouchableOpacity 
        style={styles.exploreHeader}
        onPress={() => router.push(`/(tabs)/other-profile?userId=${item.author?._id || ''}`)}
      >
        <Image 
          source={{ uri: item.author?.profilePicture || 'https://via.placeholder.com/32' }} 
      ...
      <TouchableOpacity style={styles.exploreItem}>
        <Image 
          source={{ uri: item.secure_url || 'https://via.placeholder.com/200' }} 
      ...
      {/* Caption */}
      <View style={styles.exploreCaption}>
        <Text numberOfLines={2} style={{ fontSize: 13, color: '#1a1a1a', lineHeight: 18 }}>
      ...
      <View style={styles.exploreActions}>
        <TouchableOpacity style={styles.exploreActionBtn} onPress={() => handlePostLike(item._id)}>
          <Text style={styles.exploreActionIcon}>❤️</Text>
          <Text style={styles.exploreActionCount}>{item.likes || 0}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.exploreActionBtn} onPress={() => handlePostComment(item._id)}>
          <Text style={styles.exploreActionIcon}>💬</Text>
          <Text style={styles.exploreActionCount}>{item.comments || 0}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.exploreActionBtn} onPress={() => handlePostShare(item._id)}>
          <Text style={styles.exploreActionIcon}>📤</Text>
        </TouchableOpacity>
      </View>
    </View>
    ...
    <ThemedView style={styles.container}>
      ...
      <View style={styles.contentArea}>
        ...
        {activeTab === 'flicks' ? (
          ...
          flicksLoading && flicks.length === 0 ? (
            // Show skeleton loaders while loading
            <FlatList
              data={[1, 2, 3, 4, 5]}
              renderItem={() => <FlickSkeleton />}
            ...
            <FlatList
              ...
              // ✅ CRITICAL: Performance optimizations for full-screen video content
              getItemLayout={getFlickItemLayout}
              maxToRenderPerBatch={2} // Only render 2 videos at a time (1 visible + 1 buffer)
              updateCellsBatchingPeriod={100} // Wait 100ms between batches
              removeClippedSubviews={true} // Remove off-screen video components from memory
              windowSize={3} // Render 1 above + 1 visible + 1 below
          ...
          // EXPLORE CONTENT (Card list style)
          <>
            <SearchBarComponent />
            {exploreLoading && posts.length === 0 ? (
              ...
              <FlatList
                ...
                keyExtractor={(item) => item._id}
                // ✅ CRITICAL: Performance optimizations for card-list feed
                contentContainerStyle={styles.exploreListContent}
                maxToRenderPerBatch={8} // Render 8 items at a time
                updateCellsBatchingPeriod={50} // Update every 50ms
                removeClippedSubviews={true} // Remove off-screen components
                windowSize={21} // 10 above + 1 visible + 10 below
